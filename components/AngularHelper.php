 ...
 * @version 0.3.1
 * Yii 2.x Version By Purnachandra Rao Valluri
 ...
 *
 */
class AngularHelper extends Widget
{
	...
	public $scriptsPosition = View::POS_HEAD;
	...
	public function init()
	{
		...
		if ($this->scriptsPosition === null) {
			$this->scriptsPosition = View::POS_READY;
		}
	...
	public function publishAssets()
	{
		...
		if (!empty($this->appFolder)) {
			...
			if ($this->debug || !file_exists($this->_appAssetsPath)) {
				...
				$this->_appAssetsUrl = $assetsManager->publish($this->appFolder, ['forceCopy' => true]);
	...
	public function registerScripts()
	{
		...
		if ($this->includeAngular) {			
			...
			$this->view->registerJsFile(
				$this->baseUrl .
					$this->_vendorAssets[1] . '/angular' . $jsExt,
	...
	private function _publishCommonAppScripts()
	{
		...
		foreach ($this->commonAppScripts as $scriptFile) {
			...
			$commonFileUrl = $this->_appAssetsUrl[0] . '/' . $scriptFileName;
			$commonFilePath = $scriptFileName;
	...
	{
		$content = str_replace(self::APP_ASSETS_URL_PLACEHOLDER, $this->_appAssetsUrl[0], $content);
